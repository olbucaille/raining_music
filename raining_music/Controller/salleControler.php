...
{
 		$Ids =  explode("_",$_GET['type'], 3);
		$Id = $Ids[0];
		$connexion = connect();
		$requete = $connexion->prepare("UPDATE Alerte SET Flag_lecture = 1 WHERE Id = \"".$Id."\"; ");
		if($requete->execute())
			header("location:./template/myProfile.php");
		else 
		{
			$requete = $connexion->prepare("DELETE FROM Alerte WHERE Flag_lecture = 0 AND Id = \"".$Id."\"; ");
			$requete->execute();
			header("location:./template/myProfile.php");
		}
			...
			if($type[0]=="ASKSALLE")
			{
				...
				// dans la table concert
					...
					if($requete->execute())
					{
						...
						$sql2 = "update concert set salle_acceptee = 1 where Id = '".$Id_concert."'"; // ajouter le nom de la salle dans la requette
						echo "<br>".$sql2;
